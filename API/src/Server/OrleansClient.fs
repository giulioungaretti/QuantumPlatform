...
let client(url: string) (serviceProvider:IServiceProvider) : IClusterClient =
    ...
    async {
        ...
        client.Connect( fun (ex: Exception) -> task {
            log.LogWarning("Exception while attempting to connect to Orleans cluster: {Exception}", ex);
            do! Task.Delay(1000)
            return true
        }).GetAwaiter().GetResult() |> ignore
        log.LogInformation("Orlean client initalized {%b}", client.IsInitialized)
        return client
